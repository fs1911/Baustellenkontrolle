...
async function loadUser(email: string): Promise<CurrentUser> {
  const [p] = await withService(
    (tx) => tx<{ id: string; fullName: string }[]>`select id, full_name from public.user_profiles where business_email = ${email}`,
  );
  ...
  return {
    ...
    permissions: permissionsSchema.parse(JSON.parse(perms[0].perms)),
  };
}
...
describe("Berichtsinhalt", () => {
  it("enthält Gesellschaft, Logo, Feststellungen, Massnahmen und Disclaimer", async () => {
    const built = await withUser(sibe.id, (tx) =>
      buildReportContent(tx, inspectionId, { reportNumber: "TEST-1", versionNo: 1, status: "draft", summaryText: "Test" }),
    );
...
describe("Freigabe und Versand", () => {
  it("versendet nach Freigabe an die eigene Geschäftsadresse und protokolliert alles", async () => {
    const result = await releaseAndSend(pl, inspectionId, {
      ...
      cc: [],
      bcc: [],
      subject: "Baustellenkontrollbericht – Test",
      body: "Guten Tag\n\nIm Anhang der Bericht.",
    });
    expect(result.status).toBe("sent");
    expect(result.reportNumber).toMatch(/^BHB-\d{4}-\d{4}$/);
    const [state] = await withService(
      (tx) => tx<{ status: string; isFinal: boolean; pdfSha256: string; deliveryStatus: string; sentBy: string }[]>`
    ...
    const events = await withService(
      (tx) =>
        tx<
          { action: string }[]
        >`select action from public.audit_logs where entity_type in ('generated_reports','email_deliveries') and actor_id = ${pl.id}`,
    );
  ...
  it("verweigert Projektleitenden freie externe Empfänger", async () => {
    await expect(
      releaseAndSend(pl, inspectionId, {
        summaryText: "",
        closingText: "",
        to: ["extern@fremd.example"],
        cc: [],
        bcc: [],
  ...
  it("verweigert den Versand für fremde Baustellen", async () => {
    await expect(
      releaseAndSend(pl, foreignInspectionId, {
        summaryText: "",
        closingText: "",
        to: [pl.businessEmail],
        cc: [],
  ...
  it("erlaubt Retry nur für fehlgeschlagene Aufträge", async () => {
    const [d] = await withService(
      (tx) =>
        tx<
          { id: string }[]
        >`select d.id from public.email_deliveries d join public.generated_reports r on r.id = d.report_id where r.inspection_id = ${inspectionId}`,
    );
