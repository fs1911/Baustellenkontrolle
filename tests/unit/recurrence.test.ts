...
function f(partial: Partial<AnalysisFinding> & { id: string }): AnalysisFinding {
  return {
    companyId: "c1",
    companyName: "Hochbau AG",
    siteId: "s1",
    siteName: "Wohnüberbauung Birr",
    projectId: null,
    categoryId: "absturz",
    categoryName: "Absturzsicherheit",
    subcategoryId: "absturz.seitenschutz",
    subcategoryName: "Seitenschutz fehlend oder mangelhaft",
    assessment: "negative",
    riskLevel: "high",
    responsibleRole: "Polier",
    trade: "Rohbau",
    title: "Seitenschutz fehlt",
    description: "Seitenschutz am Deckenrand fehlt",
    status: "open",
    createdAt: now,
    overdue: false,
    completedLate: false,
...
describe("Wiederholungs-Score einer Feststellung", () => {
  it("vergibt Punkte gemäss dokumentiertem Beispielmodell", () => {
    const target = f({ id: "t" });
    const history = [f({ id: "h1", createdAt: daysAgo(10), overdue: true }), f({ id: "h2", createdAt: daysAgo(30) })];
...
describe("Cluster", () => {
  ...
  it("bildet Baustellen-Cluster mit erklärtem Score und Hinweistext", () => {
    ...
    expect(site!.items.map((i) => i.criterion)).toEqual(
      expect.arrayContaining(["same_subcategory", "same_site", "high_risk", "overdue", "additional"]),
    );
...
describe("Hinweise", () => {
  it("erkennt verspätete Massnahmen je Kategorie", () => {
    const list = [
      f({ id: "1", overdue: true }),
      f({ id: "2", completedLate: true, status: "closed" }),
      f({ id: "3", overdue: true }),
      f({ id: "4" }),
    ];
