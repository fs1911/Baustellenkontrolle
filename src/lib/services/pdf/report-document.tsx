...
const s = StyleSheet.create({
  ...
  header: { position: "absolute", top: 22, left: 44, right: 44, flexDirection: "row", justifyContent: "space-between", alignItems: "center", borderBottomWidth: 1, borderBottomColor: line, paddingBottom: 8 },
  ...
  return (
    <View style={s.header} fixed>
      ...
      <View>
        ...
        <Text style={s.headerText}>{c.inspection.siteName} · {c.inspection.date}</Text>
      </View>
    </View>
  );
}
  ...
  return (
    <View style={[s.finding, { borderLeftWidth: 4, borderLeftColor: a.fg }]} wrap={f.images.length > 2}>
      <View style={[s.row, { justifyContent: "space-between", marginBottom: 4 }]} minPresenceAhead={60}>
        <Text style={[s.h3, { flex: 1, color }]}>{f.number}. {f.title}</Text>
      </View>
      <View style={[s.row, { marginBottom: 6, flexWrap: "wrap" }]}>
        <Chip label={f.assessmentLabel.toUpperCase()} bg={a.bg} fg={a.fg} />
        {f.riskLevel && <Chip label={`RISIKO: ${f.riskLabel!.toUpperCase()}`} bg={RISK_STYLE[f.riskLevel].bg} fg={RISK_STYLE[f.riskLevel].fg} />}
      ...
      {f.category && <Text style={s.small}>Kategorie: {f.category}{f.location ? ` · Ort: ${f.location}` : ""}{f.trade ? ` · Bereich: ${f.trade}` : ""}</Text>}
      {f.description && <Text style={{ marginTop: 4 }}>{f.description}</Text>}
      {f.action && (
        <View style={{ marginTop: 6 }}>
          <Text style={s.fieldLabel}>Massnahme</Text>
          <Text>{f.action}</Text>
          <Text style={[s.small, { marginTop: 2 }]}>Verantwortlich: {f.responsible ?? "–"} · Frist: {f.dueDate ?? "–"}</Text>
        </View>
      )}
      {f.references.length > 0 && (
        <View style={{ marginTop: 6 }}>
          ...
          {f.references.map((r) => (
            <Text key={r.label} style={s.small}>• {r.label}{r.reviewNote ? ` (${r.reviewNote})` : ""}</Text>
          ))}
        </View>
      )}
...
export function ReportDocument({ content: c, assets }: { content: ReportContent; assets: ReportAssets }) {
  ...
  return (
    <Document title={`Baustellenkontrollbericht ${c.reportNumber}`} author={c.company.name} subject={c.inspection.siteName} language="de-CH" creator="Baustellenkontrolle">
      {/* 1. Deckblatt */}
      <Page size="A4" style={s.page}>
        <Footer c={c} />
        <View style={{ marginTop: 10, marginBottom: 30 }}>
          {/* eslint-disable-next-line jsx-a11y/alt-text */}
          {assets.logo && <Image style={{ height: 70, maxWidth: 240, objectFit: "contain", marginBottom: 12 }} src={{ data: assets.logo, format: "png" }} />}
          <Text style={{ fontSize: 11, color: muted }}>{c.company.name}{c.company.address ? ` · ${c.company.address}` : ""}</Text>
        </View>
        ...
        <Meta label="Baustelle / Projekt" value={`${c.inspection.siteName}${c.inspection.projectName && c.inspection.projectName !== c.inspection.siteName ? ` (${c.inspection.projectName})` : ""}`} />
        ...
        <Meta label="Status des Berichts" value={c.status === "draft" || c.status === "in_review" ? "Entwurf – nicht freigegeben" : c.status === "sent" ? "Freigegeben und versendet" : "Freigegeben"} />
      ...
      <Page size="A4" style={s.page}>
        ...
        {c.counts.recurring > 0 && <Text style={[s.small, { marginBottom: 6 }]}>Wiederkehrende Abweichungshinweise: {c.counts.recurring}</Text>}
        {c.counts.overdueActions > 0 && <Text style={[s.small, { marginBottom: 6, color: "#991b1b" }]}>Überfällige Massnahmen: {c.counts.overdueActions}</Text>}
        <Text style={{ marginBottom: 14 }}>{c.summaryText || "Keine Zusammenfassung erfasst."}</Text>

        <Text style={[s.h2, { color }]} break={false}>2. Feststellungen</Text>
        {deviations.length === 0 && <Text style={s.small}>Keine Abweichungen oder Verbesserungsmöglichkeiten.</Text>}
        {deviations.map((f) => <FindingBlock key={f.id} f={f} assets={assets} color={ink} />)}

        <Text style={[s.h2, { color }]} break>3. Massnahmenübersicht</Text>
        {c.actions.length === 0 ? (
          ...
          <View>
            ...
            {c.actions.map((a, idx) => (
              <View key={`${a.findingNumber}-${idx}`} style={[s.row, { borderBottomWidth: 0.5, borderBottomColor: line, backgroundColor: a.overdue ? "#fef2f2" : idx % 2 ? "#f9fafb" : "#ffffff" }]} wrap={false}>
                ...
                <Text style={[s.td, { width: 70, fontFamily: a.overdue ? "Helvetica-Bold" : "Helvetica", color: a.overdue ? "#991b1b" : ink }]}>{a.overdue ? `Überfällig (${a.statusLabel})` : a.statusLabel}</Text>
              </View>
            ))}
        ...
        {positives.map((f) => <FindingBlock key={f.id} f={f} assets={assets} color={ink} />)}
