...
export default async function ProfilePage() {
  ...
  const d = await withUser(user.id, async (tx) => {
    ...
    const [n] = await tx<
      { emailOnAssignment: boolean; emailOnOverdue: boolean; weeklyDigest: boolean }[]
    >`select email_on_assignment, email_on_overdue, weekly_digest from public.notification_settings where user_id = ${user.id}`;
    ...
    <>
      ...
      <Card className="mb-5">
        ...
        <CardBody className="flex flex-wrap gap-2">
          {allRoles(user.permissions).map((r) => (
      ...
      <Card>
        ...
        <CardBody>
          <ProfileForm
            initial={{ fullName: user.fullName, jobTitle: user.jobTitle ?? "", phone: d.phone, email: user.businessEmail, ...d.n }}
          />
        </CardBody>
      </Card>
