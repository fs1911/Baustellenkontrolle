...
export async function saveReportDraftAction(inspectionId: string, texts: { summaryText: string; closingText: string }): Promise<ActionResult<{ versionNo: number }>> {
...
export async function suggestSummaryAction(inspectionId: string): Promise<ActionResult<{ text: string; provider: string; note: string | null }>> {
  const user = await requireUser();
  try {
...
export async function releaseAndSendAction(inspectionId: string, input: z.input<typeof sendSchema>): Promise<ActionResult<{ reportNumber: string; versionNo: number; status: "sent" | "failed"; error?: string }>> {
  const user = await requireUser();
  try {
    ...
    const r = await releaseAndSend(user, z.string().uuid().parse(inspectionId), {
      summaryText: d.summaryText, closingText: d.closingText, to: to.valid, cc: cc.valid, bcc: bcc.valid, subject: d.subject, body: d.body,
    });
    revalidatePath(`/kontrollen/${inspectionId}/bericht`);
    ...
    return {
      ok: true,
      data: r,
      message: r.status === "sent" ? `Bericht ${r.reportNumber} (Version ${r.versionNo}) freigegeben und versendet.` : `Bericht freigegeben, Versand fehlgeschlagen: ${r.error}`,
    };
  } catch (err) {
    return toUserError(err);
  }
...
export async function retryDeliveryAction(deliveryId: string, inspectionId: string): Promise<ActionResult> {
  ...
  try {
    ...
    revalidatePath(`/kontrollen/${inspectionId}/bericht`);
    return r.status === "sent" ? { ok: true, data: undefined, message: "Versand erfolgreich wiederholt." } : { ok: false, error: r.error ?? "Versand erneut fehlgeschlagen." };
  } catch (err) {
    return toUserError(err);
  }
}
