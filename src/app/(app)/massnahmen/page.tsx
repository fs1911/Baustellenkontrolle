...
export default async function ActionsPage({ searchParams }: PageProps<"/massnahmen">) {
  ...
  const cards: ActionCardData[] = await Promise.all(rows.map(async (r) => ({
    id: r.id, findingId: r.findingId, inspectionId: r.inspectionId, siteName: r.siteName, companyName: r.companyName, findingTitle: r.findingTitle,
    description: r.description, responsible: [r.responsibleRole, r.responsiblePerson].filter(Boolean).join(" – ") || null, dueDate: r.dueDate,
    status: r.status, riskLevel: r.riskLevel, overdue: r.overdue,
    updates: await Promise.all(r.updates.map(async (u) => ({
      id: u.id, comment: u.comment, statusFrom: u.statusFrom, statusTo: u.statusTo, createdAt: new Date(u.createdAt).toISOString(), authorName: u.authorName,
      attachments: await Promise.all(u.attachments.map(async (at) => ({ id: at.id, fileName: at.fileName, url: await storage().signedUrl("attachments", at.storagePath) }))),
    }))),
  })));
  ...
  return (
    <>
      ...
      <FilterBar resetHref="/massnahmen" defaultOpen={!!(f.status || f.overdue || f.siteId)}>
        <FilterSelect name="status" label="Status" value={statusParam ?? "offen"} options={[{ value: "offen", label: "Offen und in Bearbeitung" }, { value: "alle", label: "Alle" }, ...ACTION_STATUSES.map((s) => ({ value: s, label: ACTION_STATUS_LABEL[s] }))]} />
      ...
      {cards.length === 0 ? (
        <EmptyState icon={<ListChecks className="size-12" aria-hidden />} title="Keine Massnahmen" description="Für die gewählten Filter sind keine Massnahmen vorhanden – gut so." />
      ) : (
        <div className="grid gap-4 lg:grid-cols-2">
          {cards.map((c) => <ActionCard key={c.id} a={c} canVerify={canEditSite(user.permissions, siteOf.get(c.id)!)} />)}
        </div>
      )}
    </>
  );
