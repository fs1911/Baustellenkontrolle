...
export default async function ReportsPage({ searchParams }: PageProps<"/berichte">) {
  ...
  const rows = await withUser(user.id, (tx) => tx<{ id: string; inspectionId: string; reportNumber: string; status: ReportStatus; siteName: string; companyName: string; inspectedAt: Date; releasedAt: Date | null; sentAt: Date | null; versions: number; finalVersionId: string | null; deliveries: number; failed: number }[]>`
  ...
  return (
    <>
      ...
      <FilterBar resetHref="/berichte" defaultOpen={!!status}>
        <FilterSelect name="status" label="Status" value={status} options={REPORT_STATUSES.map((s) => ({ value: s, label: REPORT_STATUS_LABEL[s] }))} />
      </FilterBar>
      {rows.length === 0 ? (
        <EmptyState icon={<FileText className="size-12" aria-hidden />} title="Noch keine Berichte" description="Berichte entstehen aus einer Kontrolle über «Bericht»." />
      ) : (
        <Table caption="Berichtshistorie">
          <THead><tr><Th>Bericht-ID</Th><Th>Baustelle</Th><Th>Kontrolle</Th><Th>Status</Th><Th>Versionen</Th><Th>Versand</Th><Th>PDF</Th></tr></THead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.id}>
                <Td><Link href={`/kontrollen/${r.inspectionId}/bericht`} className="font-semibold underline">{r.reportNumber}</Link></Td>
                <Td>{r.siteName}<span className="block text-xs text-ink-muted">{r.companyName}</span></Td>
                <Td>{formatDate(r.inspectedAt)}</Td>
                <Td><ReportStatusBadge value={r.status} />{r.sentAt && <span className="block text-xs text-ink-muted">versendet {formatDateTime(r.sentAt)}</span>}</Td>
                <Td>{r.versions}</Td>
                <Td>{r.deliveries}{r.failed > 0 && <span className="block text-xs font-semibold text-negative">{r.failed} fehlgeschlagen</span>}</Td>
                <Td>{r.finalVersionId ? <a className="font-semibold text-info underline" href={`/api/reports/${r.inspectionId}/pdf?version=${r.finalVersionId}&download=1`}>Herunterladen</a> : "–"}</Td>
              </tr>
            ))}
          </tbody>
        </Table>
