  ...
  return (
    <>
      <PageHeader
        ...
        description={`${sites.length} Baustelle(n) gemäss Ihrer Berechtigung`}
        actions={<NewSiteButton companies={creatable} />}
      />
      {sites.length === 0 ? (
        <EmptyState
          icon={<Building2 className="size-12" aria-hidden />}
          title="Keine Baustellen zugeordnet"
          description="Bitte wenden Sie sich an die Administration für eine Zuordnung."
        />
      ) : (
            ...
            <Card key={s.id}>
              <CardBody className="space-y-3">
                ...
                <dl className="grid grid-cols-4 gap-2 text-center">
                  {[
                    ["Kontrollen", s.inspections],
                    ["Feststellungen", s.findings],
                    ["Offene Massn.", s.openActions],
                    ["Überfällig", s.overdue],
                  ].map(([k, v]) => (
                    <div
                      key={k as string}
                      className={`rounded-lg border p-2 ${k === "Überfällig" && (v as number) > 0 ? "border-red-300 bg-red-50" : "border-line"}`}
                    >
                ...
                <p className="text-ink-muted text-sm">Letzte Kontrolle: {formatDate(s.lastInspection)}</p>
                <div className="flex flex-wrap gap-3 text-sm font-semibold">
                  <Link className="text-info underline" href={`/kontrollen?baustelle=${s.id}`}>
                    Kontrollen
                  </Link>
                  <Link className="text-info underline" href={`/massnahmen?baustelle=${s.id}`}>
                    Massnahmen
                  </Link>
                  {analytics && (
                    <Link className="text-info underline" href={`/dashboard?baustelle=${s.id}`}>
