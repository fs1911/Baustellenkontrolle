...
export default async function RecurringPage() {
  ...
  const d = await withUser(user.id, async (tx) => {
    ...
    const insights = buildInsights(
      await loadAnalysisFindings(tx, settings["recurrence.scoring"].windowDays * 2),
      new Date(),
      settings["recurrence.scoring"],
    );
  ...
  return (
    <>
      <PageHeader
        title="Wiederkehrende Abweichungen"
        description="Datenbasierte Unterstützung zur Prävention und Verbesserung – keine Bewertung von Personen."
        actions={user.permissions.is_catalog_editor && <RecomputeButton />}
      />
      <Card className="mb-5">
        <CardHeader
          title="So wird der Trend-Score berechnet"
          description={`Betrachtungszeitraum ${c.windowDays} Tage · konfigurierbar unter Verwaltung → Einstellungen`}
        />
        <CardBody>
          <ul className="grid gap-x-6 gap-y-1 text-sm sm:grid-cols-2">
            <li>+{c.sameSubcategory} gleiche Unterkategorie im Zeitraum</li>
            <li>
              +{c.similarText} ähnliche Beschreibung (Textähnlichkeit ≥ {Math.round(c.similarityThreshold * 100)} %)
            </li>
            ...
            <li>
              +{c.perAdditional} je weitere ähnliche Feststellung (max. {c.maxAdditional})
            </li>
            {c.sameTrade > 0 && <li>+{c.sameTrade} gleicher Bereich / Gewerk</li>}
          </ul>
          <p className="text-ink-muted mt-2 text-sm">
            Priorität: hoch ab {c.highPriorityScore} Punkten, mittel ab {c.mediumPriorityScore} Punkten.
          </p>
        </CardBody>
      </Card>
      ...
      {d.clusters.length === 0 ? (
        <EmptyState
          icon={<Repeat className="size-12" aria-hidden />}
          title="Keine wiederkehrenden Muster erkannt"
          description="Sobald sich Abweichungen wiederholen, erscheinen sie hier mit nachvollziehbaren Kriterien."
        />
      ) : (
              ...
              <Card key={cl.id} id={cl.id} className={cl.status === "resolved" ? "opacity-70" : ""}>
                <CardHeader
                  ...
                  description={
                    <span className="flex flex-wrap gap-1.5">
                      <Tag>{SCOPE[cl.scope]}</Tag>
                      <Tag>{STATUS[cl.status]}</Tag>
                      <Tag>{cl.memberCount} Feststellungen</Tag>
                      {cl.overdueCount > 0 && <Tag className="text-negative">{cl.overdueCount} überfällig</Tag>}
                    </span>
                  }
                  actions={
                    <div className="text-right">
                      <p className="text-3xl font-bold tabular-nums">{cl.score}</p>
                      <p className="text-ink-muted text-xs">Trend-Score</p>
                    </div>
                  }
                />
                <CardBody className="grid gap-5 lg:grid-cols-2">
                  <div className="space-y-3">
                    ...
                    {cl.recommendation && (
                      <p className="bg-improve-soft rounded-lg p-3 text-sm font-semibold text-amber-950">{cl.recommendation}</p>
                    )}
                    <p className="text-ink-muted text-xs">
                      Zeitraum der Feststellungen: {formatDate(cl.firstSeenAt)} – {formatDate(cl.lastSeenAt)} · berechnet{" "}
                      {formatDateTime(cl.computedAt)}
                    </p>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {cl.scoreBreakdown.map((i) => (
                          <tr key={i.criterion} className="border-line border-t">
                            <td className="py-1.5 pr-2">
                              {i.label}
                              <span className="text-ink-muted block text-xs">{i.detail}</span>
                            </td>
                            <td className="py-1.5 text-right font-bold">{i.points > 0 ? `+${i.points}` : "–"}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    <details>
                      <summary className="text-info min-h-10 cursor-pointer text-sm font-semibold">
                        Zugehörige Feststellungen ({members.length})
                      </summary>
                      <ul className="mt-2 space-y-1 text-sm">
                        {members.map((m) => (
                          <li key={m.findingId}>
                            <Link className="underline" href={`/kontrollen/${m.inspectionId}/feststellungen/${m.findingId}`}>
                              {m.title}
                            </Link>{" "}
                            <span className="text-ink-muted">
                              · {m.siteName} · {formatDate(m.createdAt)}
                              {m.similarity !== null ? ` · Ähnlichkeit ${Math.round(m.similarity * 100)} %` : ""}
                            </span>
                          </li>
                        ))}
                      </ul>
