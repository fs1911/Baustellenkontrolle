...
export default async function FindingsPage({ searchParams }: PageProps<"/feststellungen">) {
  ...
  const f = {
    ...
    subcategoryId: uuidParam(sp, "unterkategorie"),
    assessment: enumParam(sp, "beurteilung", ASSESSMENTS),
  ...
  const { rows, options } = await withUser(user.id, async (tx) => ({
    rows: await searchFindings(tx, f),
    ...
    <>
      ...
      <FilterBar resetHref="/feststellungen" defaultOpen={Object.values(f).some((v) => (Array.isArray(v) ? v.length : !!v))}>
        ...
        <FilterSelect
          ...
          label="Beurteilung"
          value={f.assessment}
          options={ASSESSMENTS.map((a) => ({ value: a, label: ASSESSMENT_LABEL[a] }))}
        />
        <FilterSelect
          name="risiko"
          label="Risikostufe"
      ...
      {rows.length === 0 ? (
        <EmptyState
          icon={<Search className="size-12" aria-hidden />}
          title="Keine Feststellungen gefunden"
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="hover:bg-slate-50">
                <Td>
                  <Link href={`/kontrollen/${r.inspectionId}/feststellungen/${r.id}`} className="font-semibold underline">
                    {r.title}
                  </Link>
                </Td>
                <Td>
                ...
                <Td>
                  ...
                  {r.dueDate && <span className="text-ink-muted block text-xs">Frist {formatDate(r.dueDate)}</span>}
                </Td>
                <Td>
                  {r.categoryName ?? "–"}
                  {r.subcategoryName && <span className="text-ink-muted block text-xs">{r.subcategoryName}</span>}
                </Td>
                <Td>
                  {r.siteName}
