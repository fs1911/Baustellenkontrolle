...
export default async function CompaniesAdminPage() {
  ...
  const data: CompanyData[] = await Promise.all(
    rows.map(async (r) => ({
      ...
      defaultDistribution: r.defaultDistribution.join(", "),
      reportDisclaimer: r.reportDisclaimer ?? "",
      confidentialityNote: r.confidentialityNote ?? "",
      isActive: r.isActive,
  ...
  return (
    <>
      <PageHeader
        title="Gesellschaften, Logos und Branding"
        description="Das aktive Logo und die Markenfarbe werden automatisch in Berichten und E-Mails der Gesellschaft verwendet."
      />
        ...
        {data.map((c) => (
          <details key={c.id} className="group border-line rounded-[var(--radius-card)] border bg-white shadow-[var(--shadow-card)]">
            <summary className="flex min-h-14 cursor-pointer list-none items-center gap-3 px-5">
              ...
              <span className="text-ink-muted text-sm">{c.shortCode}</span>
              <span className="text-info ml-auto text-sm group-open:hidden">bearbeiten</span>
            </summary>
            <div className="border-line border-t p-5">
              <CompanyEditor initial={c} />
            </div>
          </details>
        ))}
        <Card>
