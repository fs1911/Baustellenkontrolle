...
export default async function CatalogPage() {
  ...
  return (
    <>
      <PageHeader
        title="Kategorien und Referenzkatalog"
        description="Referenzen sind Orientierungshilfen, keine rechtsverbindliche oder vollständige Rechtsdatenbank."
      />
      <Alert tone="warning" title={`${toReview} Referenz(en) mit Status «zu prüfen»`} className="mb-5">
        Initiale Einträge wurden ohne verifizierte Artikel-/Dokumentnummern und ohne Links angelegt. Bitte durch das IMS-/SIBE-Team prüfen,
        ergänzen (Fundstelle, Link, Version, Abrufdatum) und freigeben. In Berichten werden ungeprüfte Referenzen mit «(Zu prüfen)»
        gekennzeichnet.
      </Alert>
      ...
      <div className="mb-8 space-y-3">
        {d.categories.map((c) => {
          ...
          return (
            <details key={c.id} className="group border-line rounded-[var(--radius-card)] border bg-white shadow-[var(--shadow-card)]">
              <summary className="flex min-h-14 cursor-pointer list-none flex-wrap items-center gap-2 px-5">
                <span className="text-ink-muted">{c.sortOrder}.</span>
                <span className="font-semibold">{c.name}</span>
                <Tag>{RISK_LABEL[c.defaultRisk]}</Tag>
                <Tag>{subs.length} Unterkategorien</Tag>
                <Tag>{d.mappings.filter((m) => m.categoryId === c.id).length} Referenzen</Tag>
      ...
      <div className="space-y-3">
        {d.references.map((r) => (
          <details
            key={r.id}
            className={`group border-line rounded-[var(--radius-card)] border bg-white shadow-[var(--shadow-card)] ${r.isActive ? "" : "opacity-60"}`}
          >
            <summary className="flex min-h-14 cursor-pointer list-none flex-wrap items-center gap-2 px-5 py-2">
              ...
              <span className="text-ink-muted text-sm">{r.title}</span>
              <ReviewStatusBadge value={r.reviewStatus} />
              <Tag>Version {r.versionNo}</Tag>
              {!r.isActive && <Tag>inaktiv / ersetzt</Tag>}
            </summary>
            <div className="border-line grid gap-5 border-t p-5 lg:grid-cols-2">
              <div className="space-y-2 text-sm">
                <p>{r.description}</p>
                <p>
                  <span className="text-ink-muted">Quelle:</span> {r.source ?? "–"} · <span className="text-ink-muted">Link:</span>{" "}
                  {r.url ? (
                    <a className="text-info underline" href={r.url} target="_blank" rel="noopener noreferrer">
                      {r.url}
                    </a>
                  ) : (
                    "fehlt"
                ...
                <p>
                  <span className="text-ink-muted">Abrufdatum:</span> {formatDate(r.retrievedAt)} ·{" "}
                  <span className="text-ink-muted">Stand:</span> {r.sourceVersion ?? "–"}
                </p>
                <p>
                  <span className="text-ink-muted">Prüfung:</span>{" "}
                  {r.reviewerName ? `${r.reviewerName}, ${formatDateTime(r.reviewedAt)}` : "ausstehend"}
                  {r.reviewNote ? ` – ${r.reviewNote}` : ""}
                </p>
                <p>
                  <span className="text-ink-muted">Kategorien:</span>{" "}
                  {d.mappings
                    .filter((m) => m.legalReferenceId === r.id)
                    .map((m) => cats.find((c) => c.id === m.categoryId)?.name)
                    .filter(Boolean)
                    .join(", ") || "–"}
                </p>
              ...
              {r.isActive && (
                <ReferenceEditor
                  ...
                  initial={{
                    ...
                    categoryIds: d.mappings.filter((m) => m.legalReferenceId === r.id).map((m) => m.categoryId),
                  }}
                />
              )}
            </div>
