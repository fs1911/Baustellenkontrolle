...
export default async function AdminPage() {
  ...
  const tiles = [
    {
      href: "/admin/gesellschaften",
      label: "Gesellschaften, Logos und Branding",
      desc: "Stammdaten, Logo, Farbe, Verteiler, Disclaimer",
      Icon: Building2,
      show: admin,
    },
    {
      href: "/admin/katalog",
      label: "Kategorien und Referenzkatalog",
      desc: "Kategorien, Schlagworte, Muster-Massnahmen, Referenzen mit Prüfprozess",
      Icon: BookOpen,
      show: true,
    },
    { href: "/admin/benutzer", label: "Benutzer und Rollen", desc: "Konten, Rollen, Baustellenzuordnungen", Icon: Users, show: admin },
    {
      ...
      label: "Einstellungen",
      desc: "KI, Bilddaten, Aufbewahrung, Scoring, Berichtstexte",
      Icon: Settings,
      show: admin,
    },
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {tiles.map((t) => (
          <Link
            key={t.href}
            href={t.href}
            className="border-line hover:border-line-strong flex gap-4 rounded-[var(--radius-card)] border bg-white p-5 shadow-[var(--shadow-card)]"
          >
            <t.Icon className="text-brand size-8 shrink-0" aria-hidden />
            <span>
              <span className="block text-lg font-semibold">{t.label}</span>
              <span className="text-ink-muted text-sm">{t.desc}</span>
            </span>
          </Link>
        ))}
      </div>
