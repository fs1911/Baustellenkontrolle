...
export function KpiTile({
  ...
  href?: string;
  trend?: { delta: number; goodWhenDown: boolean };
}) {
  const toneCls = {
    neutral: "border-line",
    positive: "border-l-4 border-l-positive border-line",
    negative: "border-l-4 border-l-negative border-line",
    warning: "border-l-4 border-l-improve border-line",
  }[tone];
  const content = (
    <div
      className={cn(
        "h-full rounded-[var(--radius-card)] border bg-white p-4 shadow-[var(--shadow-card)]",
        toneCls,
...
export function Heatmap({
  ...
  cells: { categoryId: string; categoryName: string; siteId: string; siteName: string; n: number }[];
  drill: (categoryId: string, siteId: string) => string;
}) {
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full border-separate border-spacing-0.5 text-sm">
        ...
        <thead>
          <tr>
            <th scope="col" className="text-ink-muted p-2 text-left text-xs">
              Kategorie
            </th>
            {sites.map(([id, name]) => (
              <th key={id} scope="col" className="min-w-24 p-2 text-left text-xs font-semibold">
                {name}
              </th>
            ))}
          </tr>
                ...
                return (
                  <td key={sid} className="p-0">
                    {n > 0 ? (
                      <Link
                        href={drill(cid, sid)}
                        className="hover:ring-ink flex min-h-11 items-center justify-center rounded font-bold tabular-nums hover:ring-2"
                      ...
                      <span className="text-ink-subtle flex min-h-11 items-center justify-center rounded bg-slate-50" aria-label="0">
                        ·
                      </span>
                    )}
                  </td>
      ...
      <p className="text-ink-muted mt-2 text-xs">
        Farbintensität = Anzahl Abweichungen und Verbesserungsmöglichkeiten; Zahl in jeder Zelle.
      </p>
    </div>
  );
}
...
export function RiskMatrix({
  ...
  rows: { riskLevel: RiskLevel; bucket: "open" | "in_progress" | "done"; n: number }[];
  drill: (risk: RiskLevel, status?: string) => string;
}) {
  const buckets = [
    ["open", "Offen"],
    ["in_progress", "In Bearbeitung"],
    ["done", "Behoben / verifiziert / geschlossen"],
  ] as const;
  const riskCls: Record<RiskLevel, string> = {
    critical: "bg-red-800 text-white",
    high: "bg-orange-800 text-white",
    medium: "bg-amber-300 text-black",
    low: "bg-slate-200 text-black",
  };
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full border-separate border-spacing-0.5 text-sm">
        ...
        <thead>
          <tr>
            <th className="text-ink-muted p-2 text-left text-xs">Risikostufe</th>
            {buckets.map(([k, l]) => (
              <th key={k} className="p-2 text-left text-xs">
                {l}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {[...RISK_LEVELS].reverse().map((r) => (
            <tr key={r}>
              <th scope="row" className={cn("rounded p-2 text-left font-bold", riskCls[r])}>
                {RISK_LABEL[r]}
              </th>
              {buckets.map(([b]) => {
                const n = get(r, b);
                ...
                return (
                  <td key={b} className="p-0">
                    <Link
                      href={drill(r, b === "done" ? undefined : b)}
                      className={cn(
                        "flex min-h-11 items-center justify-center gap-1 rounded border font-bold tabular-nums",
                        alarm ? "border-red-400 bg-red-50 text-red-900" : "border-line bg-white",
                      )}
                    >
                      {n}
                      {alarm && <span className="sr-only"> (Handlungsbedarf)</span>}
