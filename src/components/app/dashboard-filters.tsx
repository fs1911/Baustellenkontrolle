...
export function DashboardFilterBar({ f, options, resetHref }: {
  f: DashboardFilters;
  options: { companies: { id: string; name: string }[]; sites: { id: string; name: string }[]; categories: { id: string; name: string }[]; inspectors: { id: string; name: string }[]; roles: string[] };
  resetHref: string;
}) {
  return (
    <FilterBar resetHref={resetHref}>
      ...
      <FilterSelect name="baustelle" label="Baustelle / Projekt" value={f.siteId} options={options.sites.map((s) => ({ value: s.id, label: s.name }))} />
      <FilterSelect name="typ" label="Kontrolltyp" value={f.type} options={INSPECTION_TYPES.map((t) => ({ value: t, label: INSPECTION_TYPE_LABEL[t] }))} />
      ...
      <FilterSelect name="kontrolleur" label="Kontrollierende Person" value={f.inspectorId} options={options.inspectors.map((i) => ({ value: i.id, label: i.name }))} />
    </FilterBar>
  );
}
