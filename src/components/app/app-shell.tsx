...
export function AppShell({
  ...
  return (
    <div className="flex min-h-screen flex-col md:flex-row">
      <a href="#inhalt" className="sr-only focus:not-sr-only focus:absolute focus:z-50 focus:bg-white focus:p-3">
        Zum Inhalt springen
      </a>

      {/* Desktop-Navigation */}
      <aside className="no-print bg-chrome hidden w-64 shrink-0 flex-col text-slate-200 md:flex">
        <Link href="/dashboard" className="flex items-center gap-3 px-5 py-5 text-white">
          ...
          <span className="leading-tight">
            <span className="block font-bold">Baustellenkontrolle</span>
            <span className="text-xs text-slate-400">tozzo gruppe ag</span>
          </span>
        </Link>
        {canCreate && (
          <div className="px-4 pb-3">
            <Link
              href="/kontrollen/neu"
              className="bg-brand hover:bg-brand-strong flex min-h-12 items-center justify-center gap-2 rounded-lg font-semibold text-white"
            >
          ...
          {items.map((item) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  "flex min-h-11 items-center gap-3 rounded-lg px-3 text-[15px] font-medium",
      ...
      {/* Mobile Kopfzeile */}
      <header
        className="no-print bg-chrome sticky top-0 z-30 flex items-center justify-between px-3 py-2 text-white md:hidden"
        style={{ paddingTop: "max(0.5rem, env(safe-area-inset-top))" }}
      >
        <Link href="/dashboard" className="flex items-center gap-2 font-bold">
          <span className="bg-brand flex size-9 items-center justify-center rounded-lg">
        ...
        <button
          type="button"
          className="touch-target inline-flex items-center justify-center rounded-lg"
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-40 bg-black/50 md:hidden" onClick={() => setMenuOpen(false)}>
          <nav
            aria-label="Menü"
            className="bg-chrome absolute inset-y-0 right-0 flex w-80 max-w-[85vw] flex-col text-slate-100"
          ...
          >
            ...
            <div className="flex-1 space-y-1 overflow-y-auto px-3">
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      ...
                      isActive(item.href) ? "bg-white/15 text-white" : "hover:bg-white/10",
                    )}
                  >
              ...
              <Link
                href="/profil"
                onClick={() => setMenuOpen(false)}
                className="flex min-h-12 items-center gap-3 rounded-lg px-3 hover:bg-white/10"
              >
                <UserCircle className="size-5" aria-hidden /> Profil
              </Link>
            </div>
            <form action={logoutAction} onSubmit={clearOfflineCaches} className="border-t border-white/10 p-3">
              <button type="submit" className="flex min-h-12 w-full items-center gap-3 rounded-lg px-3 hover:bg-white/10">
                <LogOut className="size-5" aria-hidden /> Abmelden
              </button>
            </form>
          </nav>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <ConnectivityBanner />
        <main id="inhalt" className="mx-auto w-full max-w-7xl flex-1 px-3 pt-4 pb-28 sm:px-5 md:px-8 md:pt-8 md:pb-10">
          {children}
        </main>
      </div>
      ...
      <nav
        ...
        className="no-print border-line fixed inset-x-0 bottom-0 z-30 grid grid-cols-4 border-t bg-white md:hidden"
      ...
      >
        ...
        {canCreate ? (
          <Link
            href="/kontrollen/neu"
            className="bg-brand flex min-h-16 flex-col items-center justify-center gap-0.5 text-xs font-semibold text-white"
          >
          ...
          <button
            type="button"
            onClick={() => setMenuOpen(true)}
            className="text-ink-muted flex min-h-16 flex-col items-center justify-center gap-0.5 text-xs font-semibold"
          >
