...
export function QuickCapture({
  ...
  inspection: { id: string; label: string; context: string };
  ...
  return (
    <div className="space-y-5">
      <p className="rounded-lg bg-slate-200 px-3 py-2 text-sm">
        <span className="font-semibold">Automatisch übernommen:</span> {inspection.context}
      </p>
      ...
      <button
        ...
        className="bg-chrome flex min-h-24 w-full items-center justify-center gap-3 rounded-xl text-2xl font-bold text-white active:bg-black"
      >
      ...
      <input
        ...
        data-testid="quick-camera-input"
      />
      {photos.length > 0 && (
        ...
        <div className="grid grid-cols-3 gap-2">
          {(
            ...
            <button
              key={v}
              type="button"
              aria-pressed={assessment === v}
              onClick={() => setAssessment(v)}
              className={cn(tile, assessment === v ? active : "border-line-strong bg-white")}
            >
          ...
          <div className="flex flex-wrap gap-2">
            {templates.map((t) => (
              <button
                ...
                aria-pressed={template?.title === t.title}
                className={cn(
                  "min-h-12 rounded-full border-2 px-4 text-left text-sm font-semibold",
                  template?.title === t.title ? "border-brand bg-brand-soft" : "border-line-strong bg-white",
                )}
              >
                {t.title}
              </button>
      ...
      <div className="space-y-2">
        <label htmlFor="quick-title" className="text-lg font-bold">
          Titel
        </label>
        <input
          id="quick-title"
          ...
          className="border-line-strong block min-h-14 w-full rounded-lg border-2 px-3 text-lg"
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {(
              ...
              <button
                ...
                aria-pressed={risk === v}
                onClick={() => setRisk(v)}
                className={cn(tile, risk === v ? active : "border-line-strong bg-white")}
              >
      ...
      <Button size="xl" className="w-full" onClick={submit} loading={pending}>
        <CheckCircle2 className="size-7" aria-hidden /> Speichern
      </Button>
      <div className="flex flex-wrap items-center justify-between gap-2 text-sm">
        <span className="text-ink-muted">{saved} in dieser Sitzung erfasst</span>
        <Link href={`/kontrollen/${inspection.id}`} className="font-semibold underline">
          Zur Kontrolle (Details ergänzen)
        </Link>
      </div>
    </div>
  );
