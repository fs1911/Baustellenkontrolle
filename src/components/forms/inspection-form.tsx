...
export function InspectionForm({
  ...
  return (
    <form onSubmit={submit} className="space-y-5" noValidate>
        ...
        <CardBody>
              ...
              {selectableCompanies.map((c) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex min-h-16 items-center gap-3 rounded-lg border-2 px-4 text-left font-semibold",
                      active ? "border-brand bg-brand-soft" : "border-line-strong bg-white hover:bg-slate-50",
                      editing && !active && "opacity-50",
                    )}
                  >
                    {active ? (
                      <Check className="text-brand size-6" aria-hidden />
                    ) : (
                      <Building2 className="text-ink-subtle size-6" aria-hidden />
                    )}
                    <span>
                      {c.name}
                      <span className="text-ink-muted block text-xs font-normal">{c.shortCode}</span>
                    </span>
                  </button>
                );
              })}
          ...
          {errors.companyId && (
            <p className="text-negative mt-2 text-sm font-semibold" role="alert">
              Bitte eine Gesellschaft wählen.
            </p>
          )}
        </CardBody>
      </Card>
      ...
      <Card>
        <CardHeader
          title="2. Baustelle / Projekt"
          actions={
            companyId &&
            creatableCompanyIds.includes(companyId) &&
            !editing && (
              <Button variant="outline" size="sm" onClick={() => setSiteDialog(true)}>
                <Plus className="size-4" aria-hidden /> Neue Baustelle
              </Button>
            )
          }
        />
        <CardBody className="space-y-4">
          ...
          {site && (
            <dl className="grid gap-2 rounded-lg bg-slate-50 p-3 text-sm sm:grid-cols-2">
              <div>
                <dt className="text-ink-muted">Baustellen-/Projektnummer</dt>
                <dd className="font-semibold">{site.siteNumber}</dd>
              </div>
              <div>
      ...
      <Card>
        ...
        <CardBody className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Kontrollierende Person" htmlFor="inspector">
            <Input id="inspector" value={inspector} readOnly disabled />
          </Field>
          ...
          <Field label="Wetter (optional)" htmlFor="weather">
            <Input id="weather" placeholder="z. B. sonnig, 18 °C" {...form.register("weather")} />
          </Field>
          <Field label="Kontrollbereich / Gewerk (optional)" htmlFor="area">
            <Input id="area" placeholder="z. B. Rohbau, Fassade" {...form.register("area")} />
          </Field>
          <Field label="Bemerkungen (optional)" htmlFor="notes" className="sm:col-span-2">
      ...
      <Card>
        <CardHeader
          title="4. Weitere anwesende Personen"
          actions={
            <Button variant="outline" size="sm" onClick={() => participants.append({ fullName: "", functionLabel: "", organisation: "" })}>
              <Plus className="size-4" aria-hidden /> Person hinzufügen
            </Button>
          }
        />
        <CardBody className="space-y-3">
          ...
          {participants.fields.map((f, i) => (
            <div key={f.id} className="border-line grid gap-2 rounded-lg border p-3 sm:grid-cols-[1fr_1fr_1fr_auto]">
              <Field label="Name" htmlFor={`p-${i}-name`} error={errors.participants?.[i]?.fullName?.message}>
                <Input id={`p-${i}-name`} {...form.register(`participants.${i}.fullName`)} />
              </Field>
              <Field label="Funktion" htmlFor={`p-${i}-fn`}>
                <Input id={`p-${i}-fn`} placeholder="z. B. Polier" {...form.register(`participants.${i}.functionLabel`)} />
              </Field>
              <Field label="Firma" htmlFor={`p-${i}-org`}>
                <Input id={`p-${i}-org`} {...form.register(`participants.${i}.organisation`)} />
              </Field>
              <Button variant="ghost" className="self-end" onClick={() => participants.remove(i)} aria-label={`Person ${i + 1} entfernen`}>
